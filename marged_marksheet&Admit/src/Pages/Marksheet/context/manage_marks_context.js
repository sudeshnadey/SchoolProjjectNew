...
const MarksheetMarkContext = ({ children }) => {
  ...
  const getData = async ({ selectedClass, selectedMarkSheet }) => {
    try {
      ...
      const response = await fetch(
        `https://api.bhattacharjeesolution.in/laravalAPI/schoolnew/api/data?class_id=${selectedClass?.class_id}&session=${updatedValue}`
      );
  ...
  const getViewMarksheet = async (props) => {
    try {
      const response = await fetch(
        ...
        `https://api.bhattacharjeesolution.in/laravalAPI/schoolnew2/viewMarksheet.php?studentId=${props?.studentId}&classId=${props?.classId}&marksheetId=${props?.marksheetId}&exam_name`
      );
...
export default MarksheetMarkContext;
