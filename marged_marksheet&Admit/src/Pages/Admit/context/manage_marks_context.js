...
const AdmitMarkContext = ({ children }) => {
  ...
  const getData = async ({ selectedClass, selectedMarkSheet }) => {
    ...
    try {
      // const response = await fetch(`https://api.bhattacharjeesolution.in/laravalAPI/schoolnew/api/data?class_id=${selectedClass?.class_id}&session=${selectedMarkSheet}`);
      const response = await fetch(
        `https://api.bhattacharjeesolution.in/laravalAPI/schoolnew/api/data?class_id=${selectedClass?.class_id}&session=${selectedMarkSheet}`
      );
...
export default AdmitMarkContext;
